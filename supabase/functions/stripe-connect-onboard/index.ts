...
serve(async (req) => {
  ...
  if (req.method === "GET") {
    ...
    const skc = Deno.env.get("STRIPE_SECRET_KEY_CONNECT");
    return new Response(
      JSON.stringify({
        STRIPE_SECRET_KEY_present: !!sk,
        STRIPE_SECRET_KEY_CONNECT_present: !!skc,
      }),
  ...
  try {
    ...
    // Always use STRIPE_SECRET_KEY_CONNECT — the Lovable-managed key does not have Connect enabled
    const stripeKey = Deno.env.get("STRIPE_SECRET_KEY_CONNECT")!;
    if (!stripeKey) {
      return new Response(JSON.stringify({ error: "STRIPE_SECRET_KEY_CONNECT is not configured" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // Hardcoded platform URLs — these never change regardless of contractor
    const return_url = "https://ezbid.pro/profile?connect=success";
    const refresh_url = "https://ezbid.pro/profile?connect=refresh";

    console.log("[stripe-connect-onboard] creating accountLink for", accountId);
    ...
    console.log("[stripe-connect-onboard] accountLink created:", accountLink.url);
