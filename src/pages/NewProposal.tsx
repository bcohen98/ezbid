  ...
  const generateProposal = async (finalItems: LineItem[]) => {
    ...
    try {
      ...
      const proposal = await createProposal({
        ...
        deposit_value: depositEnabled ? depositValue : (Number(profile?.default_deposit_percentage) || 0),
        deposit_amount: finalDepositAmount,
        balance_due: finalBalance,
        // Apply profile defaults as fallbacks when AI doesn't return content
        payment_terms: aiData.payment_terms || profile?.default_payment_terms || '',
        accepted_payment_methods: [],
        warranty_terms: aiData.warranty_terms || profile?.default_warranty || '',
        disclosures: aiData.disclosures || profile?.default_disclosures || '',
